...
const ProfileManager = (() => {
    ...
    function saveProfiles(profiles) {
        try {
            ...
            if (window.Components && Components.showToast) Components.showToast("Storage error. Profiles may not be saved.", "error");
        }
    }
    ...
    function renderProfileSelector(selectId, onchangeHandler) {
        ...
        return `
            <div class="profile-selector" style="margin-bottom: var(--space-4); display: flex; align-items: center; gap: var(--space-2);">
                <label for="${selectId}" style="color: var(--text-secondary); font-weight: 500;">Load Saved Profile:</label>
    ...
    function renderManagerUI() {
        ...
        if (profiles.length === 0) {
            profilesHtml = `
                <div class="card card-glass" style="text-align: center; padding: var(--space-8);">
                    ...
                    <h4 style="margin-bottom: var(--space-2); color: var(--text-heading);">No Profiles Saved Yet</h4>
                    <p style="color: var(--text-muted); font-size: var(--text-sm);">Add family members using the form to quickly generate their charts and dates.</p>
                </div>
            `;
        } else {
            profilesHtml = profiles.map(p => `
                <div class="card card-glass" style="margin-bottom: var(--space-3); padding: var(--space-4); display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <button class="btn btn-ghost" style="color: #ff4444; padding: 4px 8px;" onclick="ProfileManager.handleDelete('${p.id}')" aria-label="Delete Profile">
        ...
        return `
            <div style="padding: var(--space-6);">
                <h2 style="margin-bottom: var(--space-4);">Family Profiles</h2>
                <p style="color: var(--text-secondary); margin-bottom: var(--space-6);">Save birth details of family members to quickly generate their Kundali, Janma Tithi, and Muhurat calculations.</p>
                
                <div class="grid-2">
                    <!-- Add New Form -->
                    <div class="card">
                        <h3 style="margin-bottom: var(--space-4);">Add New Profile</h3>
                        <form id="profileAddForm" onsubmit="ProfileManager.handleAdd(event)">
                            <div class="form-group">
                                <label class="form-label">Name</label>
                                <input type="text" id="profName" class="form-input" placeholder="E.g., Rahul Koul" required style="width:100%">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label class="form-label">Time of Birth (Optional)</label>
                            ...
                            <div class="form-group">
                                <label class="form-label">Timezone Offset (Hours from UTC)</label>
                    ...
                    <!-- Saved Profiles List -->
                    <div>
                        <h3 style="margin-bottom: var(--space-4);">Saved Profiles</h3>
